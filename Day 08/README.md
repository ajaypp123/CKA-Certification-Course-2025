...
```bash
kubectl config # To check help
kubectl config view # Get all configuration
```
...
kubectl config use-context <context-name> # Select specific context
```
...
**Kubectl from different kubeconfig:**
```sh
kubectl --kubeconfig=/opt/configs/admin.kubeconfig get nodes
or
KUBECONFIG=/opt/configs/admin.kubeconfig kubectl get nodes
```

**Merge multiple kubeconfig:**
```sh
# ~/.kube/config, /opt/configs/admin.kubeconfig 
export KUBECONFIG=~/.kube/config:/opt/configs/admin.kubeconfig  
kubectl config view --flatten > merged-config  
mv merged-config ~/.kube/config

or

KUBECONFIG=~/.kube/config:/opt/configs/admin.kubeconfig  kubectl config view --flatten > merged-config  
mv merged-config ~/.kube/config
```
